...
CREATE TABLE empleados(
	...
	cantidadhijos TINYINT UNSIGNED CONSTRAINT CK_empleados_hijos CHECK (cantidadhijos>=0),
	estadocivil CHAR(10) CONSTRAINT CK_empleados_estadocivil CHECK (estadocivil
	IN ('casado','divorciado','soltero','viudo')),
	fechaingreso DATETIME,
	CONSTRAINT PK_empleados PRIMARY KEY (legajo),
...
CREATE OR REPLACE VIEW vista_empleados (nombreApellido, sexo, seccion, cantidad_de_hijos)
AS (SELECT CONCAT_WS(" ", nombre, apellido) AS nombreApellido, sexo, seccion, cantidadhijos FROM prueba.empleados);
...
CREATE OR REPLACE VIEW vista_empleados2 AS (SELECT * FROM empleados);
